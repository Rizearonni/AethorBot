...
class Moderation(commands.Cog):
    ...
    async def kick_slash(self, interaction: discord.Interaction, member: discord.Member, reason: str | None = None):
        try:
            ...
            await send_mod_log(self.bot, "Kick", f"{interaction.user.mention} kicked {member.mention}\nReason: {reason or 'n/a'}")
    ...
    async def ban_slash(self, interaction: discord.Interaction, member: discord.Member, delete_message_days: int = 0, reason: str | None = None):
        try:
            ...
            await send_mod_log(self.bot, "Ban", f"{interaction.user.mention} banned {member.mention}\nReason: {reason or 'n/a'}\nDelete days: {delete_message_days}")
        except Exception as e:
    ...
    async def unban_slash(self, interaction: discord.Interaction, user_id: int, reason: str | None = None):
        try:
            ...
            await send_mod_log(self.bot, "Unban", f"{interaction.user.mention} unbanned {user.mention}\nReason: {reason or 'n/a'}")
        except Exception as e:
    ...
    async def timeout_slash(self, interaction: discord.Interaction, member: discord.Member, minutes: int, reason: str | None = None):
        try:
            ...
            await send_mod_log(self.bot, "Timeout", f"{interaction.user.mention} timed out {member.mention} for {minutes}m\nReason: {reason or 'n/a'}")
        except Exception as e:
            await interaction.response.send_message(f"Failed to timeout: {e}", ephemeral=True)

    @app_commands.command(name="untimeout", description="Remove timeout from a member")
    @app_commands.default_permissions(moderate_members=True)
    async def untimeout_slash(self, interaction: discord.Interaction, member: discord.Member, reason: str | None = None):
        try:
            ...
            await send_mod_log(self.bot, "Untimeout", f"{interaction.user.mention} removed timeout for {member.mention}\nReason: {reason or 'n/a'}")
        except Exception as e:
    ...
    async def mute_slash(self, interaction: discord.Interaction, member: discord.Member, reason: str | None = None):
        ...
        try:
            ...
            await send_mod_log(self.bot, "Mute", f"{interaction.user.mention} muted {member.mention}\nReason: {reason or 'n/a'}")
        except Exception as e:
    ...
    async def unmute_slash(self, interaction: discord.Interaction, member: discord.Member, reason: str | None = None):
        ...
        try:
            ...
            await send_mod_log(self.bot, "Unmute", f"{interaction.user.mention} unmuted {member.mention}\nReason: {reason or 'n/a'}")
        except Exception as e:
    ...
    async def purge_prefix(self, ctx: commands.Context, count: int):
        try:
            ...
            await send_mod_log(self.bot, "Purge", f"{ctx.author.mention} purged {len(deleted)} messages in {ctx.channel.mention}")
        except Exception as e:
    ...
    async def purge_slash(self, interaction: discord.Interaction, count: int):
        try:
            ...
            await send_mod_log(self.bot, "Purge", f"{interaction.user.mention} purged {len(deleted)} messages in {channel.mention}")
        except Exception as e:
    ...
    async def slowmode_slash(self, interaction: discord.Interaction, seconds: int, channel: discord.TextChannel | None = None):
        ...
        try:
            ...
            await send_mod_log(self.bot, "Slowmode", f"{interaction.user.mention} set slowmode to {seconds}s in {ch.mention}")
        except Exception as e:
